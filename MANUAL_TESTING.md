...
- Scroll up, press the toolkit’s ↓, then ↩. Check nested and whole-page scrolling, including CSS `scroll-behavior: smooth`: both jumps should be instant and preserve existing inline styles. Repeat with recognized ChatGPT down arrows, including labeled and SVG-symbol-only variants and a button inside the composer form. The toolkit should replace their animated click action. Disable Return to where I was and verify native behavior returns. Unsupported icons remain untouched; toolkit ↓ remains available.
