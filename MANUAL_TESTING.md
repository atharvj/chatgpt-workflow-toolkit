...
- Check the native **Branched from** separator after the inherited answer. Its link must identify the original chat. A freshly transferred branch should send once even if mounted history, turn numbering, or citation display differs; the separator must not be treated as message text. Test both a top-level separator and one inside the response wrapper, outside its message content.
- Confirm a wrong-source separator, a quoted/hidden copy of it, or returning to the original chat never authorizes Send. An existing draft stays untouched. If branch verification fails, the error should report which check failed without exposing chat text or IDs.
...
- Highlight one sentence in a completed answer. The selection toolbar should show Ask in new chat and ☆ Bookmark below the selection without covering native Ask ChatGPT. Bookmark should open the existing label editor with only that sentence, without sending or opening a chat. Test math, a narrow viewport, selection collapsing on click, and each button setting independently. Shared/unsaved chats, user messages, and streaming answers must not offer selection bookmarking.
...
- Open ☆ beside the gear and jump to both bookmarks, including after reload: a sentence bookmark must return to its highlighted line; a whole-answer bookmark must start at the preceding user message. Check existing pre-update bookmarks too. Test a sentence deep in a long paragraph, across bold/link markup, repeated sentences, and math. If exact text cannot be located, show a notice rather than pretending the user prompt is the highlight; a unique saved paragraph may be used with an explicit notice. Rename/remove/reload; saved bookmarks should survive, while the return spot should not. Labels containing HTML must display as text.
- Confirm the toolkit dock has no duplicate ↓. Scroll up, press ChatGPT’s floating down arrow, then ↩ beside Settings. Check nested and whole-page scrolling, including CSS `scroll-behavior: smooth`: both jumps should be instant and preserve existing inline styles. Repeat with recognized ChatGPT down arrows, including labeled and SVG-symbol-only variants and a button inside the composer form. The toolkit should replace their animated click action. Disable Return to where I was and verify native behavior returns. Unsupported icons remain untouched.
...
- Disable **Jump to bottom instantly** while leaving **Return to where I was** enabled. Clicking the native down arrow should run ChatGPT’s normal scrolling animation once, without an instant competing jump; ↩ must still return to the earlier spot. Reload to verify the setting persists. Re-enable it and confirm instant jumping returns.
- Test the unlabeled floating arrow with `--thread-scroll-to-bottom-banner-offset` and `group-data-stream-active/scroll-root` classes (fixture: `tests/fixtures/native-scroll-bottom.html`). Despite its `aria-hidden` attribute, clicks on its SVG path and animated dots must save the spot, jump instantly, and allow ↩ to return. Generic wave dots, quoted HTML buttons, and unrelated download/menu arrows must remain untouched.
...
- Check narrow screens and light/dark mode: icons must stay clear of the composer, bookmark labels wrap, the panel scrolls, and Escape closes it. Use Tab/Enter to operate the label field and buttons.
